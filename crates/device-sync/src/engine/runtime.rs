...
#[cfg(test)]
mod tests {
    use super::{DeviceSyncRuntimeState, PairingFlowPhase};

    #[test]
    fn runtime_stores_pairing_flow_state() {
        let runtime = DeviceSyncRuntimeState::new();
        let flow_id = runtime.create_flow(
            "pairing-1".to_string(),
        ...
        runtime.set_flow_phase(&flow_id, PairingFlowPhase::Success);
        assert!(matches!(
            runtime.get_flow_phase(&flow_id),
            Some(PairingFlowPhase::Success)
        ));

        runtime.remove_flow(&flow_id);
        assert!(runtime.get_flow_phase(&flow_id).is_none());
    }
}
