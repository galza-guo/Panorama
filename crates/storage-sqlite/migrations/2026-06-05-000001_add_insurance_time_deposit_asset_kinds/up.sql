...
CREATE TABLE assets (
    ...
    CHECK (kind IN (
        ...
        'MPF', 'TIME_DEPOSIT', 'INSURANCE', 'PRIVATE_EQUITY', 'LIABILITY', 'OTHER',
        'FX'
    )),
...
SELECT
    ...
    CASE
        WHEN kind = 'OTHER' AND metadata IS NOT NULL
             AND (
                 lower(COALESCE(json_extract(metadata, '$.panorama_category'), '')) = 'time_deposit'
                 OR lower(COALESCE(json_extract(metadata, '$.sub_type'), '')) = 'time_deposit'
                 OR (
                     json_type(metadata, '$.start_date') = 'text'
                     AND json_type(metadata, '$.maturity_date') = 'text'
                     AND json_type(metadata, '$.principal') IS NOT NULL
                     AND (
                         json_type(metadata, '$.quoted_annual_rate') IS NOT NULL
                         OR json_type(metadata, '$.guaranteed_maturity_value') IS NOT NULL
                         OR json_type(metadata, '$.current_value_override') IS NOT NULL
                     )
                 )
             )
            THEN 'TIME_DEPOSIT'
        WHEN kind = 'OTHER' AND metadata IS NOT NULL
             AND (
                 lower(COALESCE(json_extract(metadata, '$.panorama_category'), '')) = 'insurance'
                 OR lower(COALESCE(json_extract(metadata, '$.sub_type'), '')) IN ('insurance', 'insurance_policy')
                 OR json_type(metadata, '$.insurance_provider') IS NOT NULL
                 OR json_type(metadata, '$.policy_type') IS NOT NULL
                 OR json_type(metadata, '$.payment_status') IS NOT NULL
                 OR json_type(metadata, '$.next_due_date') IS NOT NULL
             )
            THEN 'INSURANCE'
        ELSE kind
    END AS kind,
