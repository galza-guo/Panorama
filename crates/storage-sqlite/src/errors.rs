...
//! them to the database-agnostic error types defined in `panorama_core`.

use diesel::result::Error as DieselError;
use panorama_core::errors::{DatabaseError, Error};
use thiserror::Error;
...
/// `panorama_core::Error` before being returned to callers.
#[derive(Error, Debug)]
...
pub fn map_diesel_err<T>(result: std::result::Result<T, DieselError>) -> panorama_core::Result<T> {
    result.map_err(|e| e.into_core_error())
}

/// Helper function to convert an r2d2 Result to a core Result.
pub fn map_pool_err<T>(result: std::result::Result<T, r2d2::Error>) -> panorama_core::Result<T> {
    result.map_err(|e| e.into_core_error())
}
...
pub trait IntoCore<T> {
    fn into_core(self) -> panorama_core::Result<T>;
}
