...
**Goal:** Align Panorama's remaining Connect and device-sync implementation with
upstream `afadil/wealthfolio v3.2.1`, preserving only the Panorama Connect
visibility gate.

**Architecture:** Treat Connect and device sync as one upstream-owned lane.
Prefer upstream behavior over local custom runtime structure. Where upstream
code cannot be copied verbatim because Panorama's surrounding APIs have changed,
port the upstream behavior into the current interfaces with the smallest
possible diff.

**Tech Stack:** Rust (`crates/connect`, Axum server, Tauri commands), React/Vite
frontend, Vitest, Cargo.
...
- Review: `apps/frontend/src/features/panorama-local-connectors/**`
...
4. Keep
   `apps/frontend/src/features/panorama-local-connectors/components/connect-visibility-gate.tsx`
   as the only intended local divergence.
...
1. Restore upstream Connect runtime behavior that still fits current core
   interfaces.
2. Preserve current Panorama compatibility where upstream references removed
   core fields/types.
...
- Preserve:
  `apps/frontend/src/features/panorama-local-connectors/components/connect-visibility-gate.tsx`
- Preserve:
  `apps/frontend/src/features/panorama-local-connectors/components/connect-visibility-gate.test.tsx`

**Steps:**

1. Keep the Panorama visibility gate.
2. Otherwise favor upstream Connect session/auth behavior.
3. Port upstream auth behavior through current token/session plumbing where
   required.
...
2. Keep already-restored upstream security/runtime fixes (for example
   HMAC/session-proof behavior).
3. Avoid bringing in unrelated sync systems.
4. Verify with Cargo + frontend pairing/device-sync tests, update sync docs,
   commit.
...
2. Keep only compatible local adjustments required by already-accepted upstream
   security fixes.
3. Restore or replace tests so the final flow is pinned by behavior, not just by
   compilation.
4. Verify frontend tests/build, update sync docs, commit.
...
**Expected:** All commands exit successfully. Existing non-failing warnings may
remain, but no new Connect/device-sync regressions.
