...
**Goal:** Absorb high-value upstream changes through `v3.2.1` while preserving
Panorama-only customizations.

**Architecture:** Treat upstream `v3.2.1` as the reference point, then merge
selectively by module. Take low-conflict areas mostly as-is, and hand-merge
files covered by Panorama's patch inventory so CN market data, specialized
assets, compatibility behavior, and branding survive.

**Tech Stack:** Git, React, TanStack Query, Rust core/services, SQLite storage,
Tauri, Axum
...
Check every `ACTIVE` and `LOCAL_ONLY` patch row against touched upstream files
before merging code.
...
- Modify: `apps/frontend/src/features/panorama-local-connectors/**`
...
Take upstream `v3.2.1` behavior for Connect/auth/device-sync flows, then
re-apply Panorama-only visibility or branding tweaks on top.

**Step 2: Absorb selected AI assistant upgrades**

Prefer upstream fixes for attachments, context windowing, message editing,
provider UX, and model handling. Do not overwrite Panorama-specific provider
wiring or prompt/tool customizations without review.

**Step 3: Verify module boundaries**

Run targeted file diffs against `refs/tags/v3.2.1` after each sub-area to
confirm only intentional Panorama deltas remain.
...
When the same file changed upstream and locally, preserve only the documented
Panorama behavior and take the rest from upstream.

**Step 3: Re-run lane-specific checks**

Run the verification command listed in the patch inventory row immediately after
each lane is reconciled.
...
Mark what was absorbed, what remains intentionally divergent, and any follow-up
work still needed.

**Step 3: Summarize residual risk**

List any upstream areas deliberately skipped in this pass so the next sync
