...
| P-008 | ACTIVE | COMPATIBILITY | AI tooling/runtime compatibility shims for Panorama’s current core activity and income trait shapes, keeping the rest of the AI runtime aligned to upstream `v3.2.1` while adapting local import-mapping, activity search, income summary, and test-env mock signatures | `crates/ai/src/env.rs`, `crates/ai/src/tools/activities.rs`, `crates/ai/src/tools/import_csv.rs`, `crates/ai/src/tools/income.rs` | `cargo test -p wealthfolio-ai --lib`, `pnpm build:types`, `pnpm build` |
| P-007 | LOCAL_ONLY | OTHER | Connect visibility gate: keep the user-facing hide/show switch that removes Wealthfolio Connect routes and entry points without changing the upstream Connect/device-sync runtime underneath | `apps/frontend/src/features/wealthfolio-connect/components/connect-visibility-gate.tsx`, `apps/frontend/src/pages/settings/general/connect-visibility-settings.tsx`, `apps/frontend/src/routes.tsx` | `pnpm --filter frontend test -- --run`, `pnpm build` |
...
| 2026-04-16 | Connect manual snapshot guard follow-up | Restored the upstream same-day manual snapshot preservation during Connect holdings sync without introducing any new Panorama-only lane; existing patch inventory remains unchanged |
...
| 2026-04-16 | AI activity-table UI follow-up | Restored the upstream shared activity-table helpers and related activity/batch-activity UI styling without introducing any new Panorama-only lane; provider-specific forks remain documented separately under P-006 |
| 2026-04-16 | AI edit-runtime follow-up | Removed Panorama’s temporary `sourceMessageId` message-rewrite path and restored upstream parent-only edit handling across the AI frontend/runtime; no new fork-only lane was introduced and P-006 remains the only protected AI divergence |
| 2026-04-16 | AI frontend exact-alignment follow-up | Restored the remaining non-provider AI assistant frontend files and tests to the exact upstream `v3.2.1` versions; no new Panorama-only lane was introduced and P-006 remains the sole protected AI fork |
| 2026-04-16 | AI Rust exact-alignment follow-up | Restored the remaining non-provider AI Rust runtime/tooling files to the upstream `v3.2.1` versions, while keeping only the narrow compatibility shim still required because Panorama’s current core traits differ from the upstream signatures those files expect |
| 2026-04-16 | Device-sync frontend alignment follow-up | Restored the upstream hook-based device-sync frontend structure and documented the Panorama Connect visibility gate as the protected local divergence to preserve on future syncs |
| 2026-04-16 | Device-sync backend coordinator follow-up | Restored the upstream device-sync pairing coordinator/runtime wiring across the shared engine, server API, and Tauri commands without introducing any new Panorama-only patch lane; P-007 remains the only protected Connect/device-sync divergence |
| 2026-04-16 | Connect session/gating follow-up | Restored the upstream refresh-token-only Connect session payload flow and broker-sync entitlement gating while keeping P-007 as the only protected Connect/device-sync divergence; renumbered the unrelated AI compatibility shim to P-008 so future sync notes stay unambiguous |
| 2026-04-16 | Connect broker-cost/quote follow-up | Restored upstream-style fallback reuse of the previous holding average cost when brokers omit purchase price and re-enabled broker quote materialization from BUY/SELL trade prices, without introducing any new Panorama-only patch lane |
| 2026-04-16 | AI storage adapter follow-up | Removed the obsolete `delete_messages_starting_from` implementation from `crates/storage-sqlite` after the upstream parent-only edit revert; no new fork lane was introduced because the local storage adapter now simply matches the current `ChatRepositoryTrait` surface again |
| 2026-04-16 | Release audit and version alignment | Final pre-release audit fixed a QR scan CSS typo and missing pairing-dialog accessibility descriptions, then aligned Panorama app/addon/release metadata to `3.2.1`; no new fork-only patch lane was introduced |
