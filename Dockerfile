...
RUN xx-cargo build --release --manifest-path src-server/Cargo.toml && \
    ...
    cp src-server/target/$(xx-cargo --print-target-triple)/release/panorama-server /panorama-server

# Final stage
FROM alpine:3.19
WORKDIR /app
# Copy from backend (which is now build platform, but binary is target platform)
COPY --from=backend /panorama-server /usr/local/bin/panorama-server
COPY --from=frontend /web-dist ./dist
ENV WF_DB_PATH=/data/panorama.db
VOLUME ["/data"]
EXPOSE 8080
CMD ["/usr/local/bin/panorama-server"]
