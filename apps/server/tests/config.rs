use panorama_server::config::Config;
...
fn panorama_db_path_takes_priority_over_legacy_wf_db_path() {
    ...
    std::env::set_var("WF_DB_PATH", "/tmp/panorama-legacy.db");
    std::env::set_var("PANORAMA_SECRET_KEY", "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
...
fn legacy_wf_db_path_still_works_as_fallback() {
    ...
    assert_eq!(config.db_path, "/tmp/panorama-legacy.db");
    cleanup_env();
}
