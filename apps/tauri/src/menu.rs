...
pub fn create_menu<R: Runtime>(app: &AppHandle<R>) -> Result<Menu<R>, tauri::Error> {
    let app_menu = SubmenuBuilder::new(app, "Panorama")
    ...
    let help_menu = SubmenuBuilder::new(app, "Help")
        ...
        .item(&MenuItemBuilder::with_id("check_for_update", "Check for Update").build(app)?)
        .separator()
        .item(&MenuItemBuilder::with_id("show_about_dialog", "About Panorama").build(app)?)
        .build()?;
...
pub fn handle_menu_event(app: &AppHandle, instance_id: &str, event_id: &str) {
    match event_id {
        ...
        "report_issue" => {
            app.dialog()
                .message(
                    "If you encounter any issues, please report them at https://github.com/galza-guo/Panorama/issues",
                )
                .title("Report Issue")
                .show(|_| {});
        }
            ...
            tauri::async_runtime::spawn(async move {
                match crate::updater::check_for_update(app_handle.clone(), &instance_id).await {
                    ...
                    Ok(None) => {
                        ...
                        app_handle
                            .dialog()
                            .message("You're already running the latest version of Panorama.")
        ...
        "show_about_dialog" => {
            ...
            let message = format!("Panorama version {}", app_version);
            app.dialog()
                .message(message)
